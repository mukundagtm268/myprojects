...
Force Tags   vdev_write  vdev_write_err_inj_tc12
...
Setup NAS Environment
    ...
    #Set global spare disk
    NAS: Setup: Set pool hot spare disk  pool_id=${pool}

    ${diskid}=  NAS: Utils: Get pool raid disks  ${pool}  lspare=False
...
Post-stap Check Expected Pool And Drives State
    ...
    ...  checkstr=injected write error 2 times
    NAS: Utils: Check error raid disks existence  ${diskid}  count=${2}
    ...  timeout=${300}
    NAS: Utils: Check expected pool existence  ${pool}  expected_status=Warning
    ...  timeout=${600}
